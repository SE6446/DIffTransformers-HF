...
class Qwen2DifferentialAttention(Qwen2Attention):
    def __init__(self, config, layer_idx: int|None = None):
        ...
        self.lambda_k1 = nn.Parameter(torch.randn(config.num_attention_heads, self.head_dim))
        self.lambda_q2 = nn.Parameter(torch.randn(config.num_attention_heads, self.head_dim))
        self.lambda_k2 = nn.Parameter(torch.randn(config.num_attention_heads, self.head_dim))
        self.lambda_init = config.lambda_init[layer_idx]
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        #Here we chunk the tensor to perform differential attention.
        query_state1, query_state2 = query_states.chunk(2,dim=-1)
        key_state1, key_state2 = key_states.chunk(2,dim=-1)

        attn_weight1 = torch.matmul(query_state1, key_state1.transpose(2, 3)) / math.sqrt(self.head_dim/2)
        attn_weight2 = torch.matmul(query_state2, key_state2.transpose(2, 3)) / math.sqrt(self.head_dim/2)
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
            attn_weight1 = attn_weight1 + causal_mask
            attn_weight2 = attn_weight2 + causal_mask


        # upcast attention to fp32
        attn_weight1 = nn.functional.softmax(attn_weight1, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weight1 = nn.functional.dropout(attn_weight1, p=self.attention_dropout, training=self.training)
        attn_weight2 = nn.functional.softmax(attn_weight2, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weight2 = nn.functional.dropout(attn_weight2, p=self.attention_dropout, training=self.training)
        attn_weights = attn_weight1 - lambda_val * attn_weight2
        attn_output = torch.matmul(attn_weights, value_states)
